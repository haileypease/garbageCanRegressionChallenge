...
```{r}
# Subset analysis: StressSurvey ≤ 6
subsetDF <- observDF %>% filter(StressSurvey <= 6)

# Multiple regression on subset
model_subset <- lm(Anxiety ~ StressSurvey + Time, data = subsetDF)

# Diagnostic plot: Scatter plot of the subset with regression line
ggplot(subsetDF, aes(x = StressSurvey, y = Anxiety, color = Time)) +
  geom_point(size = 3, alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE, color = "red", fill = "pink", alpha = 0.3, aes(group = 1)) +
  labs(
    title = "Subset Analysis: StressSurvey ≤ 6",
    subtitle = "Diagnostic plot showing more linear relationship in this regime",
    x = "StressSurvey",
    y = "Anxiety",
    color = "Time"
  ) +
